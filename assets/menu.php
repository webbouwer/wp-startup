 ...
 * 20170830 - Oddsized - renamed prefixes and text domains and added the menu codes to the imagazine theme
  *
 * Modifications for embedding code into wp-startup plugin
 * 20180524 - Webbouwer - modified html, php and css, replaced all text-domains
 * 20180524 - Webbouwer - modified php to link to the js & css files
 * 20180524 - Webbouwer - renamed prefixes and text domains and added the code to the wp-startup plugin
 */
...
class WP_Startup_Menu_Image_Plugin {
	...
	public function menu_image_init() {
		...
		load_plugin_textdomain( 'wp-startup', false, basename( dirname( __FILE__ ) ) . '/languages' );
	}
	/**
	...
	public function menu_image_nav_menu_manage_columns( $columns ) {
		return $columns + array( 'image' => __( 'Image', 'wp-startup' ) );
	}
	/**
	 * Saving post action.
	 *
	...
	public function menu_image_add_inline_style_action() {
         ...
         *
         * Modified Webbouwer 20180524
         * Reverted plugin directory reference
         */
		//wp_register_style( 'wp-startup-menu-image', esc_url( get_template_directory_uri() ).'/assets/menu.css', array(), '1.1', 'all' );
        wp_register_style( 'wp-startup-menu-image', plugins_url( '', __FILE__ ).'/menu.css', array(), '1.1', 'all' );
		wp_enqueue_style( 'wp-startup-menu-image' );
	}
	/**
	...
	public function menu_image_admin_head_nav_menus_action() {
		...
		wp_enqueue_script( 'menu-image-admin', plugins_url( 'menu.js', __FILE__ ), array( 'jquery' ) );

		wp_localize_script(
			'menu-image-admin', 'menuImage', array(
				'l10n'     => array(
					'uploaderTitle'      => __( 'Chose menu image', 'wp-startup' ),
					'uploaderButtonText' => __( 'Select', 'wp-startup' ),
				),
				'settings' => array(
	...
	public function wp_post_thumbnail_only_html( $item_id ) {
		...
		$content      = sprintf(
			$markup,
			esc_html__( 'Menu image', 'wp-startup' ),
			$thumbnail_id ? esc_attr__( 'Change menu item image', 'wp-startup' ) : esc_attr__( 'Set menu item image', 'wp-startup' ),
			'',
			$item_id,
			$thumbnail_id ? wp_get_attachment_image( $thumbnail_id, $default_size ) : esc_html__( 'Set image', 'wp-startup' ),
			$thumbnail_id ? '<a href="#" class="remove-post-thumbnail">' . __( 'Remove', 'wp-startup' ) . '</a>' : ''
		);
		$hover_id = get_post_meta( $item_id, '_thumbnail_hover_id', true );
		$content .= sprintf(
			$markup,
			esc_html__( 'Image on hover', 'wp-startup' ),
			$hover_id ? esc_attr__( 'Change menu item image on hover', 'wp-startup' ) : esc_attr__( 'Set menu item image on hover', 'wp-startup' ),
			' hover-image',
			$item_id,
			$hover_id ? wp_get_attachment_image( $hover_id, $default_size ) : esc_html__( 'Set image on hover', 'wp-startup' ),
			$hover_id ? '<a href="#" class="remove-post-thumbnail hover-image">' . __( 'Remove', 'wp-startup' ) . '</a>' : ''
		);
		return $content;
	}
	/**
	...
	public function wp_post_thumbnail_html( $item_id ) {
		...
		<div class="menu-item-image-options">
			<p class="description description-wide">
				<label for="edit-menu-item-image-size-<?php echo $item_id; ?>"><?php _e( 'Image size', 'wp-startup' ); ?>
					<br />
					<select id="edit-menu-item-image-size-<?php echo $item_id; ?>"
						...
						<option value='full' <?php echo $image_size == 'full' ? ' selected="selected"' : '' ?>><?php _e( 'Original Size', 'wp-startup' ) ?></option>
			...
			<p class="description description-wide">
				 ...
				 * original: _e( 'Title position', 'wp-startup' );
				 */
				_e( 'Position Title & Description ', 'wp-startup' ); ?></label><br />
				<?php
				$positions = array(
					'hide'   => __( 'Hide', 'wp-startup' ),
					'above'  => __( 'Above', 'wp-startup' ),
					'below'  => __( 'Below', 'wp-startup' ),
					'before' => __( 'Before', 'wp-startup' ),
					'after'  => __( 'After', 'wp-startup' ),
				);
...
class WP_Startup_Menu_Image_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( __( '%s (Invalid)', 'wp-startup' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( __('%s (Pending)', 'wp-startup'), $item->title );
		}
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item' , 'wp-startup'); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'wp-startup'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'wp-startup'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', 'wp-startup'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php _e( 'Edit Menu Item' , 'wp-startup'); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<?php _e( 'Navigation Label', 'wp-startup' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
						<?php _e( 'Title Attribute', 'wp-startup' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						...
						<?php _e( 'Open link in a new window/tab', 'wp-startup' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
						<?php _e( 'CSS Classes (optional)', 'wp-startup' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
						<?php _e( 'Link Relationship (XFN)', 'wp-startup' ); ?><br />
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id; ?>">
						<?php _e( 'Description' , 'wp-startup'); ?><br />
                        ...
                        <?php /**
							 ...
							 */
							 ?>
                        <span class="description"><?php _e('If not blank the Description will be positioned right below the Navigation Label.', 'wp-startup'); ?></span>
				...
				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php _e( 'Move', 'wp-startup' ); ?></span>
						<a href="#" class="menus-move menus-move-up" data-dir="up"><?php _e( 'Up one', 'wp-startup' ); ?></a>
						<a href="#" class="menus-move menus-move-down" data-dir="down"><?php _e( 'Down one', 'wp-startup' ); ?></a>
						<a href="#" class="menus-move menus-move-left" data-dir="left"></a>
						<a href="#" class="menus-move menus-move-right" data-dir="right"></a>
						<a href="#" class="menus-move menus-move-top" data-dir="top"><?php _e( 'To the top', 'wp-startup' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s', 'wp-startup'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php _e( 'Remove' , 'wp-startup'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'wp-startup'); ?></a>
				</div>
